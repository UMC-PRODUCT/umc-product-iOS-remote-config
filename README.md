...
| `Editor/` | 설정 편집 macOS 앱 |

## 고치는 방법

편집 앱과 GitHub 웹 중 편한 쪽을 쓰세요. 어느 쪽이든 PR 로 올라가고 `validate` 검사를 통과해야 머지됩니다.

### 편집 앱으로 고치기 (추천)

`Editor/` 의 macOS 앱에서 폼으로 값을 고치면 PR 생성부터 검사·머지·배포 확인까지 앱이 대신 진행합니다.

#### 준비와 실행

- macOS 26 이상, Xcode(또는 Swift 6.2 이상 툴체인)
- `brew install gh` 로 `gh` 설치. 앱은 `/opt/homebrew/bin` 이나 `/usr/local/bin` 에 있는 `gh` 만 찾습니다
- `gh auth login` 으로 이 저장소에 쓰기 권한이 있는 계정에 로그인
...
Xcode 에서 `Editor/Package.swift` 를 열고 Run 해도 됩니다. 앱은 켜질 때 `main` 의 `app-config.json` 을 불러옵니다. `gh` 가 없거나 로그인이 안 돼 있으면 앱 화면에 필요한 명령이 뜹니다.

#### 사용 흐름

1. 왼쪽 사이드바에서 **강제 업데이트** 나 **화면 안내** 아래의 안내를 고릅니다. 새 안내는 **화면 안내** 옆 **+**, 복제·삭제는 안내를 우클릭
2. 폼에서 값을 고칩니다. 안내는 오른쪽 **iPhone 미리보기** 로 모양을 확인할 수 있습니다. **모양** 의 **안내** 는 `INFO`, **차단** 은 `BLOCKING` 입니다
3. 툴바의 **적용**(⌘S)을 누릅니다
4. **변경 사항 적용** 시트에서 변경 요약과 커밋 메시지(자동으로 채워지며 고칠 수 있음)를 확인하고 **적용**
...
- 불러온 뒤 다른 사람이 `app-config.json` 을 먼저 바꿨으면 적용을 멈춥니다. 툴바 **새로고침** 으로 다시 불러온 뒤 고쳐서 적용하세요. 적용하지 않은 변경은 새로고침하면 사라집니다
- 적용이 진행되는 동안에는 앱을 종료할 수 없습니다. 중간에 끊기면 브랜치나 PR 만 남기 때문입니다
...
- **배포 (GitHub Pages)** 에서 실패했다면 머지는 이미 끝나 `main` 에는 반영된 상태입니다. 저장소에서 배포 상태를 확인하세요

#### 되돌리기

- 적용 전: 툴바 **되돌리기** 로 불러온 상태로 돌아갑니다
- 적용 후: 그 PR 페이지의 **Revert** 버튼을 누르거나 편집 앱에서 값을 원래대로 고쳐 다시 적용하세요
